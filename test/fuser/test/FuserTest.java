...
public class FuserTest {
    ...
    public void concatAndSubstring() {
        ...
        assertEquals("FIREBIRD", "Select tabela.campo1 || SUBSTR(tabela.campo2 || tabela.campo3, 2, 3) || campo4 from tabela", fuser.toFIREBIRD());
    ...
    public void positionAndCharLength() {
        ...
        assertEquals("MYSQL", "SELECT SUBSTR(campo1, INSTR(campo1, 'B'), CHAR_LENGTH(campo1) - 1) FROM tabela", fuser.toMYSQL());
        assertEquals("FIREBIRD", "SELECT SUBSTR(campo1, POSITION('B' IN campo1), CHAR_LENGTH(campo1) - 1) FROM tabela", fuser.toFIREBIRD()); // TODO: Não achei o "POSITION para Firebird (Ronie)
    ...
    public void switchCase() {
        ...
        assertEquals("MYSQL", "SELECT CASE WHEN ( CONCAT(campo1, campo2)) = 'YEAH!' THEN 1 ELSE 0 END CASE FROM tabela", fuser.toMYSQL()); // TODO: End Case funcionou q é uma beleza! Boa Samuel!!!
